...
import { AdvanceDebugger } from "../../utils/AdvanceDeubgger"
...
export class ClineProvider implements vscode.WebviewViewProvider {
	...
	*/
	...
	resolveWebviewView(
		...
		// 初始化 webview 高级调试工具
		AdvanceDebugger.initWebview(webviewView.webview)
	...
	private getHtmlContent(webview: vscode.Webview): string {
		...
		*/
		...
		// 打包时 esbuild 插件会将 IS_DEV = true 替换为 IS_DEV = false
		const IS_DEV = true
		if (IS_DEV) {
			// 用 iframe 模式装载 react app，但这样就不能往 vscode 发送消息了
			const serverUrl = "http://localhost:3000/"
			return AdvanceDebugger.getDebugHtml(serverUrl)
		} else {
