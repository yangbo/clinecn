...
export class OpenRouterHandler implements ApiHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		this.client = new OpenAI({
			...
			defaultHeaders: {
				"HTTP-Referer": "https://cline.bot", // 可选，用于在 openrouter.ai 排名中包含您的应用
				"X-Title": "Cline", // 可选。显示在 openrouter.ai 排名中
			},
		})
	}
	...
	async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		...
		// 提示缓存：https://openrouter.ai/docs/prompt-caching
		// 这是专门针对 claude 模型的（某些模型可能会自动"支持提示缓存"，无需此设置）
		switch (model.id) {
